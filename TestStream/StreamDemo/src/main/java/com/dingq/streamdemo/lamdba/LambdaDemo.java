...
/**
 * @Author dingq
 * @Date 2022-07-05 21:20
 * @Version 1.0
 */
@FunctionalInterface
interface Interface1{
    /**
     * 每个方法，默认将当前对象作为参数传入到方法中
     * @param i
     * @return
     */
    int doubleNum(int i);
...
public class LambdaDemo {
    public static void main(String[] args) {
        // lambda简单理解就是把匿名内部类缩写  看看LambdaThread 和Test两个类的写法
        //lambda 的写法。使用lambda 来返回指定接口的对象实例时，要保证只有该接口只有一个要实现的抽象方法。default 默认实现方法不包含在内。
        //lambda 表达式不关心接口名是什么，方法名是什么。只关心方法的输入和输出也就是返回值。
        Interface1 interface1 = i -> i * 2; //单行有默认的return
