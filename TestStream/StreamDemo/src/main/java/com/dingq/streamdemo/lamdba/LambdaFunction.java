...
/**
 * 使用lambda进行接口方法的覆盖，并返回一个该接口的实现类对象，具体实现了该接口方法。
 *
 * @Author dingq
...
public class LambdaFunction {
    public static void main(String[] args) {
        //断言函数接口，只指定参数类型，用来对一个参数进行处理并返回boolean值。
        Predicate<String> predicate = StringUtils::isEmpty; //单行时，默认带return

        //基本类型的断言函数接口，代表输入的是一个int类型的，就不需要写泛型了。基本类型的函数接口每种函数接口都有提供
        IntPredicate predicate1 = i -> i > 0;
        predicate.test("");

        //消费函数接口，只指定参数类型，只消费，不提供，参数为一个返回
        Consumer<String> consumer = (s -> System.out.println("消费函数" + s));
        consumer.accept("");
        ...
        //提供函数接口，指定的是返回值类型，只提供消费 无参有返回值。
        Supplier<String> supplier = () -> "Supplier 提供";
        Supplier<String> supplier1 = String::new;//调用无参构造方法时的缩写，只有在调用构造方法的时候才会将方法名放到前面
        Supplier<String> supplier2 =() -> new String(); //与supplier1一样
        supplier1.get();//每次getter拿出来的对象都是不同的

        //UnaryOperator 一元函数，有输入和输出，但输入和输出类型相同。所以只需指定一个类型
